...
export async function searchSimilarChunks(
  ...
  const { data, error } = await (supabase as any).rpc('search_similar_chunks', {
...
export async function searchSimilarChunksWithRecency(
  ...
  const emailDates = new Map(
    ((emails as any) || []).map((e: any) => [e.id, new Date(e.received_at).getTime()])
  )
  ...
  const scoredResults = results.map((result) => {
    ...
    const age = emailDate ? (now - (emailDate as number)) / maxAge : 1 // Normalize to 0-1
    const recencyScore = 1 - Math.min(age, 1) // More recent = higher score

    const combinedScore = result.similarity * (1 - recencyWeight) + recencyScore * recencyWeight
